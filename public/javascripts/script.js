...
class TaniSection {
  ...
  constructor(section) {
    ...
    //単位の判定などで例外的な処理をする場合に条件分岐のために利用
    this.exception = section.dataset.exception;
    //そのセクションで必要な単位数を保持
...
class ResultManager {
  ...
  static clearResultLists() {
    ...
    while (resultMessage.length > 0) {
      result.removeChild(resultMessage[0]);
    }
  }
...
class SwitcherForHissyugaikokugo {
  ...
  switchHtmlView() {
    let englishAndChineseElements =
      // Array.from(
      //   document.getElementsByClassName("englishAndChinese"));
      [
        document.getElementById("英語"),
        document.getElementById("中国語")
      ]
    let japaneseElements =
      // Array.from(
      //   document.getElementsByClassName("japanese"));
      [document.getElementById("日本語")]
  ...
  switchTargetOfTaniCHeck() {
    let english = document.getElementById("英語");
    let chinese = document.getElementById("中国語");
    let japanese = document.getElementById("日本語");
    if (document.getElementById("NotForeignStudent").checked) {
      //学生は留学生ではないので英語と中国語を履修する
      //英語と中国語を単位の集計に加える
      english.classList.add("kubun");
      chinese.classList.add("kubun");
      //日本語を単位の集計から外す
      japanese.classList.remove("kubun");
    } else {
      //学生は留学生なので日本語を履修する
      // 英語と中国語を単位の集計から外す
      english.classList.remove("kubun");
      chinese.classList.remove("kubun");
      //日本語を単位の集計に加える
...
class ViewManager {
  ...
  adjustSideColumn() {
    ...
    // let heightOfMainHeader = document.getElementById("header").clientHeight;
